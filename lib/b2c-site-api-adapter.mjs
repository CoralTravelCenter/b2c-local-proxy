...
export class B2CSiteAPIAdapter {
    ...
    async getB2CPage() {
        if (this.b2cPage) {
            ...
            return this.b2cPage = new Promise(async resolve => {
                ...
                const jquery_defined = await page.evaluate(() => !!window.$);
                if (!jquery_defined) {
                    console.log("F*ck INCAPSULA! Waiting for navigation or reload ()");
    ...
    async queryB2C(endpoint, request_body = {}, request_method='GET') {
        const page = await this.getB2CPage();
        const req_key = JSON.stringify({endpoint, request_data: request_body, request_method});
            ...
            return page.evaluate((endpoint, request_data, request_method) => {
                return new Promise((resolve, reject) => {
                    if (request_method === 'GET') {
                        fetch(endpoint, { method: request_method }).then(response => response.text().then(text => resolve(text)));
                    } else if (request_method === 'POST') {
                        $.ajax(endpoint, {
                            method: request_method,
                            data:   request_data
                        }).done(response_data => resolve(response_data)).fail(reason => reject(reason));
                    }
                });
            }, endpoint, request_body, request_method);
        }
    }
